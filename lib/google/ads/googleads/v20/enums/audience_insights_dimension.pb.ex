defmodule Google.Ads.Googleads.V20.Enums.AudienceInsightsDimensionEnum.AudienceInsightsDimension do
  @moduledoc false

  use Protobuf, enum: true, protoc_gen_elixir_version: "0.15.0", syntax: :proto3

  field :UNSPECIFIED, 0
  field :UNKNOWN, 1
  field :CATEGORY, 2
  field :KNOWLEDGE_GRAPH, 3
  field :GEO_TARGET_COUNTRY, 4
  field :SUB_COUNTRY_LOCATION, 5
  field :YOUTUBE_CHANNEL, 6
  field :AFFINITY_USER_INTEREST, 8
  field :IN_MARKET_USER_INTEREST, 9
  field :PARENTAL_STATUS, 10
  field :INCOME_RANGE, 11
  field :AGE_RANGE, 12
  field :GENDER, 13
  field :YOUTUBE_VIDEO, 14
  field :DEVICE, 15
  field :YOUTUBE_LINEUP, 16
end

defmodule Google.Ads.Googleads.V20.Enums.AudienceInsightsDimensionEnum do
  @moduledoc false

  use Protobuf, protoc_gen_elixir_version: "0.15.0", syntax: :proto3
end
